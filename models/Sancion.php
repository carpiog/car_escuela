...
ORDER BY g.gra_orden DESC";
    ...
    public static function obtenerSancionesPorPeriodo($fechaInicio, $fechaFin) {
        $query = "SELECT 
            s.san_id,
            s.san_catalogo,
            s.san_fecha_sancion,
        ...
        LEFT JOIN armas a_ins ON m.per_arma = a_ins.arm_codigo
        WHERE s.san_situacion = 1 
        AND DATE(s.san_fecha_sancion) BETWEEN " . self::$db->quote($fechaInicio) . "::date 
        AND " . self::$db->quote($fechaFin) . "::date
